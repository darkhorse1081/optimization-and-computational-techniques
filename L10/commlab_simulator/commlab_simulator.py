...
async def send_choice(opponent_id, play, round_number):
...
async def send_acknowledgement(opponent_id, round_number):
...
async def parse_message(opponent_id, round_number):
    ...
    # -------
    # bytes :
    #     The ACTION of the message, if it is valid
    # None :
    #     If the message is invalid or should not be processed further
    #
    # Notes
    # -----
...
async def resolve(my, opp):
    ...
    await microbit.display.show(faces[diff])
    # Leave the picture up for long enough to see it
    await microbit.sleep(333)
    return result

async def display_score(score, times=3):
    ...
    if score < 9 and score >= 0:
        await microbit.display.show([screen_off, str(score)]*times, delay=333)
    elif score > 9:
        for n in range(times):
            await microbit.display.scroll(str(score))
            await microbit.display.show(screen_off)
            await microbit.sleep(333)
...
async def main():
    ...
    # select an opponent
    opponent_id = await choose_opponent()
    await microbit.sleep(333)
    
    # Run an arbitrarily long RPS contest
    while True:
        # get a play from the buttons
        choice = await choose_play()
        # send choice
        send_time = await send_choice(opponent_id, choice, round_number)
        # wait for reply
        acknowledged, resolved = (False, False)
        while not (acknowledged and resolved):
            # Handle display (no nice interrupts in single-threaded Python...)
            await microbit.display.show(microbit.Image.ALL_CLOCKS[disp % 12])
            inc += 1
            if inc > 3:
                disp += 1
                inc = 0
            # get a message from the radio
            message = await parse_message(opponent_id, round_number)
            
            # TODO: if is a play
            if False:
                # resolve the match and display the result
                result = await resolve(choice, message)
                
                # TODO: update score

                # display the score
                await display_score(score)
