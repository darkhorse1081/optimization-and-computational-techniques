...
def search(tree, search_value):
	...
	node_final = None
	last_val_arc = queue.get_node(queue.get_length()-1) # always get last node in the linked list before cycle - c0
	...
	while (not(len(unvisited_set) == 0)): # -- for check

		if (not(last_val_arc.value.arcs_out == []) and (queue.get_length() == 1)): # when daughter are there
			index = (queue.get_length())-1
			value_xf = queue.pop(index) # returns node object - if length is 1
			...
			index = (queue.get_length())-1
			value_xf = queue.pop(index) # popping produces Node class object + removes from linked list
			unvisited_set.remove(value_xf)
		...
		if len(unvisited_set) == 0:
			...
			last_val_arc = queue.get_node(queue.get_length()-1)
